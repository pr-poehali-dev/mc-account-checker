...
export default function Index() {
    ...
    try {
      ...
      return {
        ...
        players: data.players
          ? { online: data.players.online, max: data.players.max, list: data.players.list?.map((p: { name: string }) => p.name) }
          : undefined,
  ...
  return (
    <div className="min-h-screen bg-[#0a0b0f] text-white overflow-x-hidden" style={{ fontFamily: "Montserrat, sans-serif" }}>
      {/* BG glows */}
      ...
      <nav className="relative z-10 flex items-center justify-between px-6 py-4 border-b border-white/5">
        ...
        {myIP && (
          <div className="flex items-center gap-2 bg-white/5 border border-white/10 rounded-xl px-4 py-2">
            <Icon name="Globe" size={13} className="text-[#22c55e]" />
      ...
      <div className="relative z-10 text-center pt-14 pb-8 px-6">
        ...
        <p className="text-gray-400 text-base max-w-xl mx-auto leading-relaxed">
          Наш сервис работает уже более шести лет на рынке, обеспечивая вашу безопасность.<br />
          ...
          <div className="bg-[#111318] border border-white/5 rounded-2xl p-6">
              ...
              <div className="flex flex-col md:flex-row gap-6 items-start">
                ...
                <div className="flex-1 space-y-3">
                  <div className="flex items-center gap-3 mb-3">
                    <img src={playerInfo.headUrl} alt="head" className="w-10 h-10 rounded-lg" />
                    <div>
                      <div className="text-white font-black text-xl">{playerInfo.name}</div>
                      <div className="text-[#22c55e] text-xs font-semibold">✓ Аккаунт найден в базе Mojang</div>
                    </div>
                  </div>
                  <InfoRow label="Никнейм" value={playerInfo.name} icon="User" />
                  <InfoRow label="UUID" value={uuidToDashed(playerInfo.id)} icon="Hash" mono />
                  <InfoRow label="Скин" value="Скачать скин" icon="Image" link={`https://crafatar.com/skins/${playerInfo.id}`} />
                  <InfoRow label="История ников" value="Открыть на NameMC" icon="History" link={`https://namemc.com/profile/${playerInfo.name}`} />
                  <InfoRow label="Плащ" value="Проверить плащ" icon="Layers" link={`https://crafatar.com/capes/${playerInfo.id}`} />
                </div>
              </div>
            )}
            ...
            {!playerInfo && !playerError && !playerLoading && (
              <div className="text-center py-10 text-gray-600">
                ...
                <p className="text-xs mt-1 text-gray-700">Данные берутся из официального Mojang API</p>
              </div>
            )}
          </div>
        )}
          ...
          <div className="bg-[#111318] border border-white/5 rounded-2xl p-6">
            <div className="flex gap-3 mb-4">
              <input
                ...
                placeholder="IP сервера: hypixel.net или 185.0.0.1:25565"
            ...
            {serverStatus && (
              <>
                ...
                {/* Status */}
                <div className={`flex items-center gap-3 rounded-xl px-5 py-4 mb-4 border ${serverStatus.online ? "bg-[#22c55e]/10 border-[#22c55e]/30" : "bg-[#ef4444]/10 border-[#ef4444]/30"}`}>
                  <span className={`w-3 h-3 rounded-full flex-shrink-0 ${serverStatus.online ? "bg-[#22c55e] animate-pulse" : "bg-[#ef4444]"}`} />
                ...
                <div className="grid grid-cols-2 gap-3 mb-4">
                  <InfoRow label="Хост" value={serverStatus.host} icon="Globe" mono />
                  ...
                  {serverStatus.players && (
                    <InfoRow label="Игроки" value={`${serverStatus.players.online} / ${serverStatus.players.max}`} icon="Users" />
                  )}
                ...
                {serverStatus.players?.list && serverStatus.players.list.length > 0 && (
                  <div className="bg-[#0d0e13] border border-white/5 rounded-xl p-4 mb-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {serverStatus.players.list.map(p => (
                        <span key={p} className="bg-[#111318] border border-white/10 text-white text-xs font-mono px-3 py-1.5 rounded-lg">{p}</span>
                      ))}
                    </div>
                  </div>
                )}
                ...
                <div className="flex items-end gap-1 h-14 mb-3">
                  {[...monitor].reverse().map((e, i) => (
                    <div key={i} className="flex flex-col items-center flex-1">
                      <div
                        className={`w-full rounded-sm ${e.online ? "bg-[#22c55e]" : "bg-[#ef4444]/50"}`}
                        style={{ height: `${e.online ? Math.max(8, Math.min(52, (e.players / (serverStatus?.players?.max || 100)) * 52 + 8)) : 4}px` }}
                      />
                    </div>
                  ))}
                </div>
            ...
            {!serverStatus && !serverError && !serverLoading && (
              <div className="text-center py-10 text-gray-600">
                ...
                <p className="text-sm">Введите IP-адрес или домен Minecraft-сервера</p>
                <p className="text-xs mt-1 text-gray-700">Поддерживаются Java и Bedrock серверы</p>
              </div>
            )}
          </div>
        )}
